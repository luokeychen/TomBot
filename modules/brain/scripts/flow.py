...
class Flow(Engine):
    ...
    def respond(self, message, matches):
        ...
        if self.check_contain_chinese(key):
            gevent.spawn(message.send, self.querydb(serial=None, title='%'+key+'%'))
            gevent.sleep(0)
        else:
            gevent.spawn(message.send, self.querydb(serial=key, title=''))
            gevent.sleep(0)

    def querydb(self, serial, title):
        ...
        if result:
            ...
            result = self.gbk2utf8(' '.join(result[1:]))
            result += '\n' + url
        else:
            result = 'Noting found.'
        cs.close()
